...
registerMooseObject("PF_learningApp", HystreTabulatedFluidProperties);

InputParameters
HystreTabulatedFluidProperties::validParams()
{
...
HystreTabulatedFluidProperties::HystreTabulatedFluidProperties(const InputParameters & parameters)
  : HystreSinglePhaseFluidProperties(parameters),
    ...
    _fp(getUserObject<HystreSinglePhaseFluidProperties>("fp")),
...
HystreTabulatedFluidProperties::~HystreTabulatedFluidProperties() {}

void
HystreTabulatedFluidProperties::initialSetup()
{
          ...
          _required_columns.end())
      {
        if (std::find(_property_columns.begin(), _property_columns.end(), column_names[i]) ==
          ...
          mooseError(
              ...
              " read in ",
              _file_name,
              " is not one of the properties that HystreTabulatedFluidProperties understands");
        else
  ...
  // HystreTabulatedFluidProperties. Now set flags and indexes for each property in
...
std::string
HystreTabulatedFluidProperties::fluidName() const
{
  return _fp.fluidName();
}

Real
HystreTabulatedFluidProperties::molarMass() const
{
  return _fp.molarMass();
}

Real
HystreTabulatedFluidProperties::rho_from_p_T(Real pressure, Real temperature) const
{
...
void
HystreTabulatedFluidProperties::rho_from_p_T(
...
Real
HystreTabulatedFluidProperties::e_from_p_T(Real pressure, Real temperature) const
{
...
void
HystreTabulatedFluidProperties::e_from_p_T(
...
Real
HystreTabulatedFluidProperties::h_from_p_T(Real pressure, Real temperature) const
{
...
void
HystreTabulatedFluidProperties::h_from_p_T(
...
Real
HystreTabulatedFluidProperties::mu_from_p_T(Real pressure, Real temperature) const
{
...
void
HystreTabulatedFluidProperties::mu_from_p_T(
...
Real
HystreTabulatedFluidProperties::c_from_p_T(Real pressure, Real temperature) const
{
  return _fp.c_from_p_T(pressure, temperature);
}

Real
HystreTabulatedFluidProperties::cp_from_p_T(Real pressure, Real temperature) const
{
  if (_interpolate_cp)
  {
    checkInputVariables(pressure, temperature);
...
Real
HystreTabulatedFluidProperties::cv_from_p_T(Real pressure, Real temperature) const
{
  if (_interpolate_cv)
  {
    checkInputVariables(pressure, temperature);
...
Real
HystreTabulatedFluidProperties::k_from_p_T(Real pressure, Real temperature) const
{
  if (_interpolate_k)
  {
    checkInputVariables(pressure, temperature);
...
void
HystreTabulatedFluidProperties::k_from_p_T(
...
Real
HystreTabulatedFluidProperties::s_from_p_T(Real pressure, Real temperature) const
{
...
void
HystreTabulatedFluidProperties::s_from_p_T(
    Real p, Real T, Real & s, Real & ds_dp, Real & ds_dT) const
{
  HystreSinglePhaseFluidProperties::s_from_p_T(p, T, s, ds_dp, ds_dT);
}

std::vector<Real>
HystreTabulatedFluidProperties::henryCoefficients() const
{
  return _fp.henryCoefficients();
}

Real
HystreTabulatedFluidProperties::vaporPressure(Real temperature) const
{
  return _fp.vaporPressure(temperature);
}

void
HystreTabulatedFluidProperties::vaporPressure(Real temperature, Real & psat, Real & dpsat_dT) const
{
  _fp.vaporPressure(temperature, psat, dpsat_dT);
}

void
HystreTabulatedFluidProperties::writeTabulatedData(std::string file_name)
{
  if (processor_id() == 0)
  {
    ...
    file_out << "# " << _fp.fluidName()
             << " properties created by HystreTabulatedFluidProperties on " << ctime(&now) << "\n";
...
void
HystreTabulatedFluidProperties::generateTabulatedData()
{
...
void
HystreTabulatedFluidProperties::reshapeData2D(unsigned int nrow,
                                              unsigned int ncol,
...
void
HystreTabulatedFluidProperties::checkInputVariables(Real & pressure, Real & temperature) const
{
...
Real
HystreTabulatedFluidProperties::z_from_p_T(Real p, Real T) const
{
  checkInputVariables(p, T);
  return _fp.z_from_p_T(p, T);
}

void
HystreTabulatedFluidProperties::z_from_p_T(
    Real p, Real T, Real & z, Real & dz_dp, Real & dz_dT) const
{
  checkInputVariables(p, T);
  _fp.z_from_p_T(p, T, z, dz_dp, dz_dT);
}

Real
HystreTabulatedFluidProperties::f_from_p_T(Real p, Real T) const
{
  checkInputVariables(p, T);
  return _fp.f_from_p_T(p, T);
}

void
HystreTabulatedFluidProperties::f_from_p_T(
    Real p, Real T, Real & f, Real & df_dp, Real & df_dT) const
{
  checkInputVariables(p, T);
  _fp.f_from_p_T(p, T, f, df_dp, df_dT);
}

Real
HystreTabulatedFluidProperties::psi_from_p_T(Real p, Real T) const
{
  checkInputVariables(p, T);
  return _fp.f_from_p_T(p, T) / p;
}

void
HystreTabulatedFluidProperties::psi_from_p_T(
    Real p, Real T, Real & psi, Real & dpsi_dp, Real & dpsi_dT) const
{
  checkInputVariables(p, T);
  _fp.psi_from_p_T(p, T, psi, dpsi_dp, dpsi_dT);
}
