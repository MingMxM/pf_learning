...
[Functions]
  ...
  [insitu_temperature]
    type = ParsedFunction
    expression = '${temp} + (${depth} - y) * ${geothermal_gradient}'
  []
  [injection_rate]
    type = ParsedFunction
    expression = '-${inject_mass}/(2 * pi * ${bh_r} * (${max_y} - ${min_y}) * ${inject_time_s})'
  []
  [production_rate]
    type = ParsedFunction
    expression = '${produce_mass}/(2 * pi * ${bh_r} * (${max_y} - ${min_y}) * ${produce_time_s})'
  []
[]
